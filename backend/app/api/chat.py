...
async def switch_model(data: SwitchModelRequest):
    """Switch to a different model (pulls from Ollama if needed)."""
    success = await ollama_manager.switch_model(data.model_id)
    if not success:
        raise HTTPException(
            status_code=503,
            detail=ollama_manager.error or "Failed to switch model",
        )
    return ModelStatus(**ollama_manager.get_status())
...
async def chat(
    ...
    if not await ollama_manager.is_ready():
